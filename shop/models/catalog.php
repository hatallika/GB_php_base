...
function getCart ($session_id) {
    return getAssocResult("SELECT c.id as cart_id, p.id, p.name, p.price, p.image , c.quantity 
...
function deleteFromCart($id){
    executeSql("DELETE FROM cart WHERE id = {$id}"); //Сессия не нужна так как id уникальный
}
